...
+ (NSDictionary *)indexed:(NSArray *)data {
    ...
    for (NSString *k in allKeys) {
        ...
        NSArray *a =[[NSMutableArray alloc]initWithArray: [enumerator allObjects]];
...
+ (NSString *)getIndexWithFirstChar:(NSString *)fc language:(NSString *)lang {
    ...
    //Case Insensitive 不区分大小写   Ascend 升序
    if ([fc localizedCaseInsensitiveCompare:first] != NSOrderedAscending &&
        ...
        if([fc localizedCaseInsensitiveCompare:@"重"] == NSOrderedSame || [fc isEqualToString:@"长"]){
            i = 2;
        }
        if ([fc isEqualToString:@"八"] || [fc isEqualToString:@"巴"]) {
            i = 1;
        }
        if ([fc isEqualToString:@"阿"] || [fc localizedCaseInsensitiveCompare:@"阿"] == NSOrderedSame) {
            i = 0;
        }
    }
    if (i != III_NOT_FOUND) {
