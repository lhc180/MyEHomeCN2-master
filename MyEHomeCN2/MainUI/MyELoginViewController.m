...
- (void)viewDidLoad
{
    ...
    self.usernameInput.inputAccessoryView = nil;
    self.passwordInput.inputAccessoryView = nil;
...
-(void)deletePrefsInPlist{
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    NSMutableArray *users = [self getUsersFromPlist];
    NSMutableArray *usersCache = [users mutableCopy];
    for (NSDictionary *d in users) {
        if ([d[@"username"] isEqualToString:[prefs objectForKey:@"username"]]) {
            if ([usersCache containsObject:d]) {
